...
def train_and_evaluate_models(_X_integrated, _X_analytics, _data):
    ...
    if models_exist:
        ...
        integrated_cv = cross_val_score(best_drop_model_int, X_train_int, y_train_int, cv=5).mean()
        ...
        analytics_cv = cross_val_score(best_drop_model_ana, X_train_ana, y_train_ana, cv=5).mean()  # cross validation using only the training set
